    ...
    T: Clone + Eq + Hash,
{
    ...
    fn merge(&mut self, other: PnSet<T>) {
        for (element, other_count) in other.elements.into_iter() {
            let self_count = self.elements
                .entry(element)
                .or_insert_with(|| HashMap::new());
            for (replica_id, pn) in other_count.into_iter() {
                self_count.entry(replica_id).or_insert(Pn::new()).merge(pn);
            }
        }
    }
...
impl<T: Eq + Hash> PartialOrd for PnSet<T> {
    fn partial_cmp(&self, other: &PnSet<T>) -> Option<Ordering> {
        fn a_gt_b(a: &HashMap<ReplicaId, Pn>, b: &HashMap<ReplicaId, Pn>) -> bool {
            a.len() > b.len() || a.iter().any(|(replica_id, a_pn)| {
                b.get(replica_id)
                    .map_or(true, |b_pn| a_pn.p > b_pn.p || a_pn.n > b_pn.n)
            })
        }

        let self_is_greater = self.elements.iter().any(|(element, counts)| {
            other
                .elements
                .get(element)
        ...
        let other_is_greater = other.elements.iter().any(|(element, counts)| {
            self.elements
                .get(element)
                .map_or(true, |other_counts| a_gt_b(counts, other_counts))
        });
...
#[cfg(any(quickcheck, test))]
impl<T> Arbitrary for PnSet<T>
where
    T: Arbitrary + Clone + Eq + Hash,
{
    fn arbitrary<G>(g: &mut G) -> PnSet<T>
    where
        G: Gen,
    {
        use gen_replica_id;
    ...
    fn shrink(&self) -> Box<Iterator<Item = PnSet<T>> + 'static> {
        ...
        Box::new(self.elements.shrink().map(move |es| {
            PnSet {
...
#[cfg(any(quickcheck, test))]
impl<T> Arbitrary for PnSetOp<T>
where
    T: Arbitrary,
{
    fn arbitrary<G>(g: &mut G) -> PnSetOp<T>
    where
        G: Gen,
    {
        PnSetOp {
    ...
    fn shrink(&self) -> Box<Iterator<Item = PnSetOp<T>> + 'static> {
        ...
        Box::new((element, replica_id, pn).shrink().map(
            |(element, replica_id, pn)| {
                PnSetOp {
...
mod test {
    ...
    fn test_ordering_lt() {
        ...
        quickcheck(check_ordering_lt as fn(PnSet<u8>, PnSet<u8>) -> bool);
    }
}
